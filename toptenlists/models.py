...
class Item(models.Model):
    """Models for toptenlist items
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    modified_at = models.DateTimeField(auto_now_add=True)
    name = models.CharField(max_length=255, blank=True, default='')
    description = models.CharField(max_length=5000, blank=True, default='')
    toptenlist = models.ForeignKey(TopTenList, on_delete=models.CASCADE, related_name='item')
    order = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(10)])

    class Meta:
        # unique_together = ('toptenlist', 'order') # not using this because it prevents items from being swapped because deferred is not available in mysql
        ordering = ['order']
